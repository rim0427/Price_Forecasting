...
## **1차 예선: 기본 데이터 분석 및 초기 모델링 접근 결정**

### **주요 분석 및 결정 사항**
1. **통합 모델링 vs. 품목별 모델링**
   - **통합 모델링의 장점**:
     - 모든 품목을 하나의 모델로 학습하면, **데이터 양이 많아져 일반화 성능**이 향상될 가능성 있음.
     - 다양한 품목의 데이터를 동시에 학습하며, 공통된 패턴을 반영할 수 있음.
   - **EDA 결과**:
     - 각 품목은 고유한 **계절성 패턴**과 **가격 변동 특성**을 보임.
     - 예시:
       - **배추**: 김장철에 수요 급증으로 가격이 상승.
       - **감자**: 여름철 기후 변화에 더 큰 영향을 받음.
     - 통합 모델에서는 이러한 **특이한 패턴**이 반영되지 않아, **예측 정확도 저하** 우려.
   - **결정**: 품목별 모델링을 채택하여, 각 품목의 고유한 계절성과 변동성을 효과적으로 학습.

2. **동일 품목 내 다양한 품종 데이터 처리**
   - **문제 정의**:
     - 예를 들어, '감자' 품목 내에서도 **'수미', '대지'**와 같은 여러 품종이 존재.
     - 동일 품목 내 품종 간의 데이터 차이를 어떻게 처리할지 고민.
   - **가정 및 검증**:
     - 동일 품목 내 품종들은 **비슷한 재배 환경과 계절성**을 가질 것이라 가정.
     - **UMAP 분석**과 **시간별 가격 변동 EDA**를 통해 검증한 결과, 동일 품목 내 품종들이 **유사한 군집 형성** 확인.
   - **결정**: 품종 데이터를 **다변량 입력 형식**으로 통합해 학습. 이를 통해 데이터 양 증가 및 모델의 일반화 성능 강화.

3. **모델 선택: 머신러닝 모델 vs. 시계열 딥러닝 모델**
   - **머신러닝 모델의 장점**:
     - 다양한 외부 요인(기후, 경제적 변수 등)과 비선형 관계를 학습 가능.
     - **랜덤 포레스트, LGBM** 등 다양한 머신러닝 모델을 활용해 **장기 예측**에 강점.
   - **시계열 딥러닝 모델의 장점**:
     - **LSTM, Transformer** 모델은 시간적 흐름과 **계절성 패턴** 학습에 유리.
     - 단기 예측에서 **최근 시점의 패턴**을 효과적으로 반영 가능.
   - **결정**: **앙상블 전략** 채택:
     - **단기 예측**에는 시계열 딥러닝 모델의 가중치를 더 부여.
     - **장기 예측**에는 머신러닝 모델의 가중치를 더 부여해 예측 성능 극대화.
