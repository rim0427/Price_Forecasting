...
def store_data(test_dome, test_jointmarket, deal_info, folder_name='for_infer'):
    """
    test_dome, test_jointmarket, deal_info 데이터를 지정된 폴더에 저장하는 함수.
    
    Args:
        test_dome (DataFrame): 저장할 test_dome 데이터.
        test_jointmarket (DataFrame): 저장할 test_jointmarket 데이터.
        deal_info (DataFrame): 저장할 deal_info 데이터.
        folder_name (str): 데이터를 저장할 폴더 이름. 기본값은 'for_infer'.
    """
    # 스크립트 파일 경로 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def train_dlin1(품목리스트, config):
    ...
    for item in 품목리스트:
        ...
        # 데이터 파일 및 메타 파일 경로 설정
        script_dir = os.path.dirname(os.path.abspath(__file__))
        if item in group1:
            train_file = os.path.join(script_dir, "../data/train/train_1.csv")
            meta_file = os.path.join(script_dir, "../data/train/meta/TRAIN_경락정보_가락도매_2018-2022.csv")
        elif item in group2:
            train_file = os.path.join(script_dir, "../data/train/train_2.csv")
            meta_file = os.path.join(script_dir, "../data/train/meta/TRAIN_중도매_2018-2022.csv")
        elif item in group3:
            train_file = os.path.join(script_dir, "../data/train/train_2.csv")
            meta_file = os.path.join(script_dir, "../data/train/meta/TRAIN_소매_2018-2022.csv")
        ...
        model_save_path = os.path.join('dl_weights2', f'{item}_model1_win3.pth')
...
def train_dlin2(품목리스트, config):
    ...
    for item in 품목리스트:
        ...
        os.makedirs('dl_weights2', exist_ok=True)
        model_save_path = os.path.join('dl_weights2', f'{item}_model2_win9.pth')
        torch.save(model.state_dict(), model_save_path)
        print(f"Model for {item} saved at {model_save_path}")
...
def save_scalers(scalers, folder_name='for_infer'):
    """
    스케일러들을 지정된 폴더에 저장하는 함수.
    
    Args:
        scalers (dict): 저장할 스케일러 딕셔너리.
        folder_name (str): 저장할 폴더 이름. 기본값은 'for_infer'.
    """
    # 현재 스크립트의 디렉토리 경로 설정
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_dir = os.path.join(script_dir, folder_name)
    ...
    print(f"Scalers have been saved to '{scalers_path}'")


# 스케일러 딕셔너리 생성
scalers = {
    'potato_scaler1': potato_scaler1,
    'pepper_scaler1': pepper_scaler1,
    'garlic_scaler1': garlic_scaler1,
    'daepa_scaler1': daepa_scaler1,
    'moo_scaler1': moo_scaler1,
    'cabbage_scaler1': cabbage_scaler1,
    'apple_scaler1': apple_scaler1,
    'lettuce_scaler1': lettuce_scaler1,
    'onion_scaler1': onion_scaler1,
    'pear_scaler1': pear_scaler1,
    
    'pepper_scaler2': pepper_scaler2,
    'garlic_scaler2': garlic_scaler2,
    'daepa_scaler2': daepa_scaler2,
    'moo_scaler2': moo_scaler2,
    'cabbage_scaler2': cabbage_scaler2,
    'apple_scaler2': apple_scaler2,
    'lettuce_scaler2': lettuce_scaler2,
    'onion_scaler2': onion_scaler2,
    'pear_scaler2': pear_scaler2,
...
print(f"추론 완료 (소요 시간: {elapsed_time:.4f}초)")
...
preds_dict = {
    ...
    'pear': (pear_preds1, None),  # 장기 예측 없음
}
