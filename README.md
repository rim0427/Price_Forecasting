...
이제 전체 내용이 요약되었으며, 핵심 포인트들이 간결하고 명확하게 정리되었습니다.


# ```본선```   


본선 진출 이후, 저희는 예측 모델을 활용한 **‘AI 기반 수매-방출 의사결정 시스템’**을 개발했습니다. 이 시스템은 농산물 시장의 가격 안정화를 목표로 설계되었으며, 농민, 소비자, 정부 모두에게 이익이 되는 솔루션을 제공합니다.
...
- **수확 가능성 피처 (`harvest_weight`)**
  - 농산물의 수확 시기는 가격 변동의 중요한 요인입니다. 따라서 품목별로 수확 가능한 월을 기준으로 수확 가능성을 나타내는 피처를 추가했습니다. 이는 특정 시기에 가격 급등/급락을 설명하는 데 도움이 됩니다.
- **Low-Pass Filter 적용**
...
- **경제 지표 필터링 및 제거**
  - 불필요하거나 관련성이 낮은 경제 지표(`수출`, `수입` 등)를 제거해, 데이터의 노이즈를 줄이고 모델의 성능을 향상시켰습니다.
...
- **이동 평균 피처 생성**
  - 최근 3개월 간의 이동 평균을 계산해, 단기적인 가격 변동성을 완화하고 스무딩된 가격 패턴을 반영했습니다. 이는 단기 예측의 안정성을 높이는 데 기여합니다.
- **Target Encoding 및 표준편차 인코딩**
  - `달_순` 및 `수확 가능성`을 기준으로 가격의 평균과 표준편차를 계산하여 인코딩 피처를 생성했습니다. 이를 통해 계절별, 수확기별 가격 패턴의 차이를 반영했습니다.
...
- **외부 데이터 필터링**
  - 품목별로 관련성이 낮은 외부 데이터를 제거해, 모델이 중요한 변수에 집중할 수 있도록 했습니다. 예를 들어, `수출`, `수입` 데이터를 제거함으로써 모델링 과정에서의 노이즈를 줄였습니다.

---

### **핵심 요약**
- **자연적 요인:** 누적 기상 데이터와 수확 가능성, Low-Pass Filter 등을 활용해 환경적 영향을 반영.
- **경제적 요인:** 총반입량과 개입 확률을 통해 시장의 공급/수요 요인을 반영.
- **시계열적 요인:** 가격 차이, 이동 평균 등을 통해 시간 흐름에 따른 변동성을 학습.
- **외부적 요인:** 정부 개입 및 기후 변동을 반영해, 외부 요인에 따른 가격 변동성을 예측.

이제 모든 피처 엔지니어링 과정이 각 요인별로 설명되었고, 그 목적과 의도도 명확히 반영되었습니다.
```
